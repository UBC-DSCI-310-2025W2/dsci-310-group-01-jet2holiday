...
Based on the Adjusted R-squared value, the model yielded an adjusted r-squared of `r round(summary(airbnb_model)$adj.r.squared, 3)` meaning that the selected five predictors were able to successfully explain 49.1% of all variance in the log-transformed prices. The outputted F-statistic (`r round(summary(airbnb_model)$fstatistic[1], 2)`, p < `r round(summary(airbnb_model)$fstatistic[2], 2)`) indicates that this model is highly significant. Every predictor was statistically significant with only `room_typeShared room` and `property_typeEntire rental unit` sitting at the highest p-values but were still well below the standard `r round(summary(airbnb_model)$coefficients[1], 5)` threshold. 
